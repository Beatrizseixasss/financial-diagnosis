    ...
    <section id="flashcards" ref={sectionRef} className={`flashcards ${!isVisible ? "flashcards--hidden" : ""}`}>
      <div className="container mx-auto px-6">
        ...
        <p className="section-lead mb-8 text-lg text-muted-foreground">
          Responda perguntas rápidas sobre dinheiro, crédito, juros e investimentos. Acerte para ganhar XP, erre e você
          perde vidas.
        </p>
        ...
        <div className={`flash-card ${cardState}`}>
            ...
            <div>
              ...
              <div className="options">
                {(currentQuestion?.options || []).map((opt, idx) => {
                  ...
                  const showIncorrect = selectedOption !== null && isSelectedOption && !isCorrectOption

                  return (
                    <button
                      key={idx}
                      type="button"
                      className={`option-btn ${showCorrect ? "correct-answer" : ""} ${showIncorrect ? "wrong-answer" : ""}`}
          ...
          <div className="flash-card-footer">
            <p className="feedback-text">
              {feedback ||
                (isDefaultMode ? "Assim que você escolher um caminho, as perguntas aparecem aqui." : pigMessage)}
            </p>
            {showNextLevelBtn && (
